...
const esc = (s) => {
  ...
  return _escEl.innerHTML.replace(_escRe, (c) => _escMap[c]);
};
...
function parseMarkdown(text) {
  ...
  body = body.replace(/((?:^\|.+\|$\n?)+)/gm, (block) => {
    ...
    const tbody = bodyRows.length
      ? `<tbody>${bodyRows.map((r) => `<tr>${parseCells(r, "td")}</tr>`).join("")}</tbody>`
      : "";
  ...
  body = body.replace(
    /(<\/h[34]>|<\/table>|<\/blockquote>|<\/pre>|<\/ul>|<hr>)\s*<\/p>/g,
    "$1",
  );
  return body;
}
...
function renderStructuredTags(data) {
  ...
  if (s) {
    const sm = s.sentiment || "中性",
      rk = s.riskLevel || "中";
    const smC =
      { 看涨: "badge-up", 看跌: "badge-down", 分化: "badge-mix" }[sm] ||
      "badge-flat";
    const rkC =
      { 高: "badge-risk-h", 低: "badge-risk-l" }[rk] || "badge-risk-m";
...
function renderAnalysis(data) {
  if (!data || !data.analysis)
    return '<div class="empty"><div class="empty-icon">📭</div><p>暂无分析数据</p></div>';
  const date =
    data.date ||
    (data.generatedAt
...
function renderNewsItem(it) {
  ...
  const t = href
    ? `<a href="${esc(href)}" target="_blank" rel="noopener">${esc(it.title)}</a>`
    : esc(it.title);
...
async function loadMeta() {
  ...
  try {
    const r = await fetchT("data/news/meta.json");
    _meta = await r.json();
  } catch (e) {
    _meta = { categories: [] };
  }
...
async function loadAllDates() {
  ...
  try {
    _allDates = await fetchT("data/news/analysis/index.json").then((r) =>
      r.json(),
    );
  } catch (e) {
    _allDates = [];
